#ifndef PSO_REPULSION_H
#define PSO_REPULSION_H
...
class laser_ray
{
	public:
		...
		float repulsion_start = 3.0; // [m], distance at which we start exerting repulsion
		float repulsion_max = 1.0; // [m], distance at which we hit maximum repulsion (in a normal case)
		float repulsion_min = 0.1; // [m/s], repulsion in the direction of the laser
		float repulsion_max = 1.0; // [m/s], repulsion in the direction of the laser
};

class PSO_REPULSION: public Controller
{
public:
	PSO_REPULSION():Controller(){};
